function Q = cinv(T, R, q_kuka_kr, mejor)
    if nargin == 3
        mejor = false;
    elseif nargin ~= 4
    ...
    Td = invHomog(R.base.double) * Td * invHomog(R.tool.double);

    %Eliminación de offsets --------------------------------------------------
    offsets_originales = R.offset;
    R.offset = zeros(6,1);

    %Punto (x,y,z) de la muñeca ----------------------------------------------
    p = Td(1:3,4) - R.links(6).d * Td(1:3,3);
    ...
    for i=1:2:7
        ...
        q2 = qq(2, i);
        q3 = qq(3, i);
        [q4, q5, q6] = calcular_qm(R, q1, q2, q3, Td, q_kuka_kr);
        qq(4:6,i:i+1) = [q4; q5; q6];
    end
    % Offset ------------------------------------------------------------------
    R.offset = offsets_originales;
...
r = sqrt(p(1)^2 + p(2)^2);
L2 = R.links(2).a; % a2
L3 = sqrt(R.links(4).d^2+R.links(3).a^2); % raiz d4^2+a3^2
...
q3aux = atan2(p(2), p(1));
q3= q3aux-atan2(R.links(4).d,R.links(3).a);
end

%=========================================================================%
% Cálculo de q4,q5,q6
function [q4,q5,q6] = calcular_qm(R, q1, q2, q3, T, q_kuka_kr)
T1 = R.links(1).A(q1).double;
